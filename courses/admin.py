...
class SubscriptionPlanMonthInline(admin.TabularInline):
    ...
    verbose_name_plural = "Mois inclus (annuel ou tranche)"


@admin.register(SubscriptionPlan)
class SubscriptionPlanAdmin(admin.ModelAdmin):
    list_display = ("name", "billing_period", "months_summary", "amount", "is_active")
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        main = (
            None,
            {
                "fields": ("name", "billing_period", "amount", "is_active"),
                "description": (
                    "<strong>Mensuel</strong> : ne remplissez pas la liste de mois. "
                    "Le mois accordé est celui du sujet / corrigé / examen choisi par le candidat.<br>"
                    "<strong>Annuel</strong> ou <strong>Tranche</strong> : indiquez le mois de départ "
                    "puis les mois inclus dans la section ci-dessous (mois + année facultative)."
                ),
            },
        )
        if obj and obj.is_monthly_by_content():
            return (main,)
        period = obj.billing_period if obj else request.POST.get("billing_period")
        if period == SubscriptionPlan.BillingPeriod.MONTHLY:
            return (main,)
        period = (
            "Mois de départ (administrateur)",
            {
                "fields": ("start_year", "start_month", "included_months"),
                "description": (
                    "Définissez l’<strong>année</strong> et le <strong>mois de départ</strong> : "
                    "les mois accordés partent de cette date (liste ci-dessous ou mois consécutifs). "
                    "Le candidat ne choisit pas le mois de départ pour l’annuel ou une tranche."
                ),
            },
        )
        return (main, period)
    ...
    def get_inlines(self, request, obj=None):
        if obj and obj.is_monthly_by_content():
            return []
        return [SubscriptionPlanMonthInline]

    def save_related(self, request, form, formsets, change):
        super().save_related(request, form, formsets, change)
        obj = form.instance
        if obj.is_monthly_by_content():
            obj.included_periods.all().delete()
            return
        from django.core.exceptions import ValidationError

        try:
            obj.validate_months_configured()
        except ValidationError as exc:
            for msg in exc.messages:
                messages.error(request, msg)
            raise

    @admin.display(description="Mois / période")
    def months_summary(self, obj):
        if not obj.pk:
            return "—"
        if obj.is_monthly_by_content():
            return "Mois du contenu (automatique)"
        if obj.included_periods.exists():
            return obj.months_display()
        if obj.billing_period == obj.BillingPeriod.YEARLY:
            return f"{obj.included_months} mois consécutifs"
        return "—"
...
class SubscriptionRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Demande d’abonnement",
            {
                ...
                "description": (
                    "Les mois accordés pour l’<strong>annuel</strong> ou une <strong>tranche</strong> "
                    "sont définis dans l’option d’abonnement (mois de départ administrateur). "
                    "Pour le <strong>mensuel</strong>, vérifiez le mois du contenu choisi par le candidat."
                ),
            },
        ),
        (
    ...
    def approve_requests(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{count} demande(s) approuvée(s). Les mois inclus proviennent de la formule "
            f"(mois de départ défini par l’administrateur).",
            level=admin_messages.SUCCESS,
        )
