...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        is_formateur_home = user_can_access_content_formateur_space(request.user)
    return render(
        request,
        "courses/home.html",
        {
            ...
            "is_formateur_home": is_formateur_home,
        },
    )
...
def category_months(request, slug):
    ...
    if request.user.is_authenticated:
        ...
        if user_can_access_content_formateur_space(request.user):
            try:
                ensure_formateur_category_access(
                    request.user, category, assigned_only=True
                )
            except PermissionDenied:
                messages.error(
                    request,
                    "Cette catégorie ne vous est pas assignée.",
                )
                return redirect("courses:home")
...
def exam_quiz(request, category_slug, year, month, pk):
    ...
    return render(
        ...
        "courses/interactive_quiz.html",
        {
            ...
            "quiz_duration_minutes": exam.duration_minutes,
            "quiz_time_remaining_seconds": exam_time_remaining_seconds(request, exam),
...
def correction_quiz(request, category_slug, year, month, pk):
    ...
    return render(
        ...
        "courses/interactive_quiz.html",
        {
            ...
            "pdf_inline_url_name": "courses:correction_detail",
        },
    )
...
def subscribe(request):
    ...
    if not initial_category:
        messages.info(
            request,
            "Choisissez d’abord une catégorie depuis l’accueil, "
    ...
    plans = form.plans_for_display
    if initial_period:
        access_label = (
            f"{initial_category.name} — "
            f"{content_month_period_label(initial_period[0], initial_period[1])}"
        )
    else:
        access_label = initial_category.name
    return render(
        request,
