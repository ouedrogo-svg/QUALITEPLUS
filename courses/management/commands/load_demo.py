...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not monthly.name:
            monthly.name = "Mensuel"
        monthly.included_months = 1
        monthly.save(update_fields=["name", "included_months"])
        ...
        if not yearly.name:
            yearly.name = "Annuel (12 mois)"
        if yearly.included_months < 2:
            yearly.included_months = 12
        today = timezone.localdate()
        if not yearly.start_year:
            yearly.start_year = today.year
        if not yearly.start_month:
            yearly.start_month = today.month
        yearly.save(
            update_fields=["name", "included_months", "start_year", "start_month"]
        )
