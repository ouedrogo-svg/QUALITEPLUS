...
class SubscriptionRequestForm(forms.ModelForm):
    content_choice = forms.ChoiceField(
        ...
        help_text="Catégorie et mois du contenu (sujet, correction ou examen) visé.",
    )
    ...
    def __init__(self, *args, user=None, initial_category=None, initial_period=None, **kwargs):
        ...
        self.plan_only = bool(initial_category)
        super().__init__(*args, **kwargs)
        if self.plan_only:
        ...
        self.fields["plan"].queryset = self._available_plans()
        self.fields["plan"].label = "Tranche d’abonnement"
        self.fields["plan"].empty_label = None

    @property
    def plans_for_display(self):
        """Plans actifs avec libellé des mois (affichage template)."""
        ref_year = self.initial_period[0] if self.initial_period else None
        plans = list(self.fields["plan"].queryset)
        for plan in plans:
            if plan.is_monthly_by_content():
                plan.subscribe_months_label = plan.months_display(ref_year)
            else:
                plan.subscribe_months_label = plan.months_display()
        return plans

    def _available_plans(self):
        return SubscriptionPlan.objects.filter(is_active=True).prefetch_related(
            "included_periods"
        )
    ...
    def clean(self):
        ...
        if not self.user or not category or not plan:
            return cleaned
        if plan.is_monthly_by_content() and (year is None or month is None):
            raise forms.ValidationError(
                "Pour la formule mensuelle, ouvrez un mois depuis la catégorie "
                "puis cliquez sur « S’abonner » sur ce mois."
            )
        content_year = year if plan.is_monthly_by_content() else None
        content_month = month if plan.is_monthly_by_content() else None
        req_year, req_month = request_period_for_plan(
            plan, content_year=content_year, content_month=content_month
        )
        label = subscription_access_label(category, req_year, req_month, plan)
        periods = plan_covered_periods(plan, content_year, content_month)
        if not periods:
            raise forms.ValidationError(
                "Cette formule n’est pas correctement configurée. Contactez l’administrateur."
            )
        ...
        if plan.is_monthly_by_content():
            pending_qs = pending_qs.filter(year=req_year, month=req_month)
...
class SubscriptionRequestAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, formateur_user=None, **kwargs):
        ...
        if plan and plan.uses_defined_months():
            if "year" in self.fields:
                self.fields["year"].disabled = True
                self.fields["year"].help_text = (
                    "Défini dans l’option d’abonnement (mois de départ administrateur)."
                )
            if "month" in self.fields:
                ...
                self.fields["month"].help_text = (
                    "Les mois accordés sont ceux configurés dans la formule annuelle ou tranche."
                )
        elif "year" in self.fields:
            self.fields["year"].label = "Année du contenu"
            self.fields["year"].help_text = "Mois du sujet, corrigé ou examen visé (formule mensuelle)."
        if "month" in self.fields and not (
            plan and plan.uses_defined_months()
        ):
            self.fields["month"].label = "Mois du contenu"
            self.fields["month"].widget = forms.Select(choices=MONTH_CHOICES)
        if "plan" in self.fields:
            self.fields["plan"].help_text = (
                "Les mois inclus et le montant sont définis dans l’option d’abonnement."
            )
    ...
    def clean(self):
        ...
        plan = cleaned.get("plan")
        if plan:
            cy = year if plan.is_monthly_by_content() else None
            cm = month if plan.is_monthly_by_content() else None
            periods = plan_covered_periods(plan, cy, cm)
            if not periods:
                raise ValidationError(
                    "Impossible de calculer la période accordée : vérifiez la formule "
                    "(mois de départ et mois inclus)."
                )
            if not plan.is_monthly_by_content():
                req_year, req_month = request_period_for_plan(
                    plan, content_year=cy, content_month=cm
                )
                cleaned["year"] = req_year
                cleaned["month"] = req_month
